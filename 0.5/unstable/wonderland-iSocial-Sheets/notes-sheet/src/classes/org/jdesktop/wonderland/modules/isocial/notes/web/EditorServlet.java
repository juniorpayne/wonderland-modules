 ...
 */
package org.jdesktop.wonderland.modules.isocial.notes.web;
...
public class EditorServlet extends ISocialServletBase {
    ...
    {
        String editor = null;
        if (sheet.getDetails() instanceof NotesSheet) {
            editor = "/edit.jsp";
        } else {
        ...
        throws IOException, ServletException
    {
        if (sheet.getDetails() instanceof NotesSheet) {
            ...
            ((NotesSheet) sheet.getDetails()).setAutoOpen(Boolean.parseBoolean(autoOpen));
        } else {
            throw new WebApplicationException(Response.Status.BAD_REQUEST);
        }
