 ...
 */
package org.jdesktop.wonderland.modules.isocial.viewprev.web;
...
public class EditorServlet extends ISocialServletBase {
    ...
    {
        String editor = null;
        if (sheet.getDetails() instanceof SetValueSheet ||
            sheet.getDetails() instanceof GetValueSheet)
        {
            editor = "/edit.jsp";
        } else {
        ...
        throws IOException, ServletException
    {
        ...
        if (sheet.getDetails() instanceof SetValueSheet) {
            ((SetValueSheet) sheet.getDetails()).setInternalName(name);
        } else if (sheet.getDetails() instanceof GetValueSheet) {
            ((GetValueSheet) sheet.getDetails()).setInternalName(name);
        } else {
            throw new WebApplicationException(Response.Status.BAD_REQUEST);
        }
