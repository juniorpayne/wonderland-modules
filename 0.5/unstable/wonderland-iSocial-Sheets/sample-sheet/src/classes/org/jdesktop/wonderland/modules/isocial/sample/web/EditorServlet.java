 ...
 */
package org.jdesktop.wonderland.modules.isocial.sample.web;
...
public class EditorServlet extends ISocialServletBase {
    ...
    {
        String editor = null;
        if (sheet.getDetails() instanceof SampleSheet) {
            editor = "/edit.jsp";
        } else {
        ...
        throws IOException, ServletException
    {
        if (sheet.getDetails() instanceof SampleSheet) {
            ...
            ((SampleSheet) sheet.getDetails()).setQuestion(question);

            String name = request.getParameter("name");
            ((SampleSheet) sheet.getDetails()).setName(name);

            String autoOpen = request.getParameter("autoOpen");
            ((SampleSheet) sheet.getDetails()).setAutoOpen(Boolean.parseBoolean(autoOpen));
        } else {
            throw new WebApplicationException(Response.Status.BAD_REQUEST);
        }
