 ...
 * @license CC-BY-3.0 */
"use strict";
var decodedFilesNum = 0;
var decodedFilesToShow = [];
var fileIndexNotToPlay = [];
var audioSources;
var counter = 0;
var filesToView;
var WaveSurfer = {
    ...
    loadFiles: function(counterL, filesToViewL) {
        counter = counterL;
        filesToView = filesToViewL;
        this.loadFile1(filesToView[counter]);
    },
    
    loadArrayBuffers: function (a0) {
        this.decodeArrayBuffer(a0, function (a1) {
            console.log("array buffer decode,,,");
            decodedFilesNum++;
//            if (decodedFilesNum !== filesToView.length) {
                decodedFilesToShow.push(a1);
//            }
            if (decodedFilesNum === filesToView.length) {
                console.log("Ok done decoding...");
                this.drawer.drawMultipleWaves(decodedFilesToShow, this, a1);
            } else {
                counter++;
                this.loadFile1(filesToView[counter], filesToView);
            }
        }.bind(this));
    },
    
    loadFile1: function(fileURL) {
        ...
        d.on("success", function (a0) {
            c.loadArrayBuffers(a0);
        });
...
WaveSurfer.create = function(a) {
        ...
        getPeaks: function(a, buf) {
            for (var b = buf.length / a, c = ~~(b / 10) || 1, d = buf.numberOfChannels, e = [], f = [], g = 0; d > g; g++)
                for (var h = e[g] = [], i = buf.getChannelData(g), j = 0; a > j; j++) {
        ...
        disconnectSource: function() {
            if(audioSources) {
                for(var i in audioSources) {
                    audioSources[i].disconnect();
                }
            }
        },
        destroy: function() {
            this.isPaused() || this.pause(), this.unAll(), this.buffers = null, this.disconnectFilters(), this.disconnectSource(), this.gainNode.disconnect(), this.scriptNode.disconnect(), this.analyser.disconnect()
        },
        load: function(a) {
            if(this.buffers.length===(filesToView.length))
                return;
        ...
        getDuration: function() {
            return this.buffers ? this.buffers[0].duration : 0
        },
        seekTo: function(a, b) {
        ...
        play: function(a, b) {
            //create sources and connect them to destination
            audioSources = new Array();
            for (var i in decodedFilesToShow) {
                if (fileIndexNotToPlay.indexOf(parseInt(i)) === -1) {
                    audioSources[i] = this.ac.createBufferSource();
                    audioSources[i].buffer = decodedFilesToShow[i];
                    audioSources[i].connect(this.analyser);
                }
            }
        ...
        pause: function() {
            ...
            //stop all audio
            for(var i in audioSources) {
                audioSources && audioSources[i].stop(0);
            }
        ...
        load: function(a, b, c) {
            ...
            f && b.removeChild(f), b.appendChild(e), this.media = e, this.peaks = c, this.onPlayEnd = null, this.buffers = null, this.setPlaybackRate(this.playbackRate)
        },
        isPaused: function() {
        ...
        getPeaks: function(a) {
            return this.buffers ? WaveSurfer.WebAudio.getPeaks.call(this, a) : this.peaks || []
        },
        getVolume: function() {
        ...
        handleEventNew: function(a) {
            a.preventDefault();
            var b = this.wrapper.getBoundingClientRect();
            var left = (a.clientX - b.left + this.wrapper.scrollLeft) / this.wrapper.scrollWidth || 0
            var top = a.clientY - b.top;
        ...
        updateHeight: function(a) {
            this.wrapper.style.height = a + "px";
        },
        progress: function(a) {
        ...
        updateSize: function() {
            console.log("updateSize..."+this.height);
            var a = Math.round(this.width / this.params.pixelRatio);
            this.waveCc.canvas.width = this.width, this.waveCc.canvas.height = totalwaveformsHeight , this.style(this.waveCc.canvas, {
                width: a + "px"
            }), this.style(this.progressWave, {
                display: "block"
            }), this.progressCc && (this.progressCc.canvas.width = this.width, this.progressCc.canvas.height = this.height+30 , this.style(this.progressCc.canvas, {
                width: a + "px"
            })), this.clearWave()
        },
        clearWave: function() {
    ...
    drawWave: function (a, b) {
        ...
        console.log("this.params.waveColor : "+this.params.waveColor);
        this.waveCc.fillStyle = this.params.waveColor, this.progressCc && (this.progressCc.fillStyle = this.params.progressColor), [this.waveCc, this.progressCc].forEach(function (b) {
            if (b) {
        ...
        drawMultipleWaves: function(decodedFilesToShow, thisWaveSurfer) {
            console.log("drawMultipleWaves..."+decodedFilesToShow.length);
            var thisDrawer = this;
            this.setHeight(decodedFilesToShow.length * (this.params.height * this.params.pixelRatio));
            var a9,b9,c9,d9,a,f=soundStickerHeight;
            [this.waveCc, this.progressCc].forEach(function(b) {
                if (b) {
                    
                    for(var idx=0;idx<decodedFilesToShow.length;idx++) {
                            thisWaveSurfer.backend.load(decodedFilesToShow[idx]);
                        a9 = Math.round(thisWaveSurfer.getDuration() * thisWaveSurfer.params.minPxPerSec * thisWaveSurfer.params.pixelRatio);
                        b9 = thisDrawer.getWidth();
                        c9 = a9;
                        thisWaveSurfer.params.fillParent && (!thisWaveSurfer.params.scrollParent || b9 > a9) && (c9 = b9);
                        console.log("decodedFilesToShow : "+decodedFilesToShow[idx].length);
                        d9 = thisWaveSurfer.backend.getPeaks(c9, decodedFilesToShow[idx]);
                        thisDrawer.resetScroll();
                        if(idx===0)
                            thisDrawer.setWidth(c9);
                        
                        a = d9;
                        
                        var d = .5 / thisDrawer.params.pixelRatio,
                        ...
                        this.waveCc.fillStyle = colors[idx];
                        ...
                        b.closePath();
                        b.fill();
                        b.fillRect(0, g + f - d, this.width, d);
                        f = f + waveformsHeight[idx];
                        thisWaveSurfer.fireEvent("redraw", d9, c9);
                    }
                }
            }, this)
            thisWaveSurfer.fireEvent("ready");
        },
        updateProgress: function(a) {
